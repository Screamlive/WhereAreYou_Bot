...
- `TOKEN` не должен храниться в git; для production рекомендуется `EnvironmentFile`.
...
- `TOKEN` хранится вне репозитория.
...
- проект еще не полностью избавился от прямых импортов `config`, поэтому одного env-файла без `config.py` недостаточно;
